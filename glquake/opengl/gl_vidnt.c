...
qboolean VID_SetWindowedMode (int modenum)
{
	...
	CenterWindow(dibwindow, WindowRect.right - WindowRect.left, WindowRect.bottom - WindowRect.top);
...
void CheckMultiTextureExtensions (void) 
{
	...
	Con_Printf("Multitexture Not Found\n\n");
}
...
void GL_Init (void)
{
	...
	while (*p)
	{
		...
		// print one extension
		char extname[2048];
		if (len >= sizeof(extname)) len = sizeof(extname) - 1;
		memcpy(extname, q, len);
		extname[len] = '\0';

		Con_Printf("  %s\n", extname);

		// skip past this extension and any spaces
		p = q + len;
		while (*p == ' ') p++;
	}
...
void VID_Init (unsigned char *palette)
{
	...
	hIcon = LoadIcon (global_hInstance, MAKEINTRESOURCE (ID_ICON));
...
void VID_MenuDraw (void)
{
	...
	M_Print (24, 52, "Video modes must be set from the");
	M_Print (24, 60, "command line with -width <width>");
	M_Print (24, 68, "and -bpp <bits-per-pixel>");
	M_Print (24, 84, "Select windowed mode with -window");
}
